...
function processTransactions(api, parentOp, parentPath, topLevel, options) {
  ...
  parentOp.getResponses().forEach(function (res, ndx, arr) {
    ...
    transactions.push({
      ...
      'host': topLevel.host,
      'basePath': topLevel.basePath,
      'subPath': params.path,
      'pathParams': params.pathExamples,
      'op': parentOp.method,
      'body': requestBody,
      'query': params.query,
      ...
      'headers': merge2(params.headers,
          processHeaders(res.statusCode, parentOp, parentPath, topLevel,
              options)),
      'validationExcludedAttributes': options.validationExcludedAttributes
    })
  })
...
function processParams(code, op, path, options) {
  ...
  op.getParameters().forEach(function (param, ndx, arr) {
    ...
    if (param.in === 'body') {
      ...
      const pathExamples = param.getExamples()
      params.pathExamples[param.name] = pathExamples !== undefined ? pathExamples 
          : pathify(params.path, param, customValue)
    } else if (param.in === 'header') {
      const headerExamples = param.getExamples()
      Object.keys(headerExamples).forEach((exampleName) => {
        if (code === exampleName) {
          params.headers[param.name] = headerExamples[exampleName].value
        }
      })
      params.headers[param.name] = params.headers[param.name] ? params.headers[param.name] : customValue
    }
  })
...
/**
 * Processes the request body of an operation for use in a test
 * @function processRequestBody
 * @memberof processing
 * @param {*} op sway Operation object that's being processed
 * @returns {processing.ProcessedParams}
 */
function processRequestBody(op) {
  var body = {}
  var requestBodyObject = op.getRequestBody()
  if (requestBodyObject && requestBodyObject.hasOwnProperty('content')) {
      Object.keys(requestBodyObject.content).forEach((mimeType) => {
          if (requestBodyObject.content[mimeType].hasOwnProperty('examples')) {
              Object.keys(requestBodyObject.content[mimeType].examples).forEach((example) => {
                  body[example] = requestBodyObject.content[mimeType].examples[example].value
              })
          }
      });
  }
...
function processResponse(res, op, path, options) {
  ...
  var examples
  if (res.definition.hasOwnProperty('content')) {
      expected['res'] = {}
      Object.keys(res.definition.content).forEach((mimeType) => {
          examples = res.getExamples(mimeType)
          if (examples) {
              Object.keys(examples).forEach((example) => {
                  expected['res'][example] = examples[example].value
              })
          }
      })
